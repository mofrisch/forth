...
t( nz 23 nq 1/7 ncommon-type q_type = swap dup nvalue qdrop ndrop swap nvaldrop q 23/1 q= and )

n-mem-debug [if] n-show-mem [then]

t( nq 1/8 nz 17 ncommon-type q_type = swap nvaldrop q 17/1 q= rot dup nvalue qdrop ndrop and )
t( nz 56 nfr 9.01 ncommon-type fr_type = swap dup nvalue frdrop ndrop swap nvaldrop fr 56 eps fr~abs and )
t( nq 1/2 nfr 3.14 ncommon-type fr_type = swap dup nvalue frdrop ndrop swap nvaldrop fr 0.5 eps fr~abs and )

n-mem-debug [if] n-show-mem [then]
t( nfr 4.5 nz 37 ncommon-type fr_type = swap nvaldrop fr 37 eps fr~abs rot dup nvalue frdrop ndrop and )
t( nfr 17.3 nq 1/3 ncommon-type fr_type = swap nvaldrop q 1/3 q>fr eps fr~abs rot dup nvalue frdrop ndrop and )
totals
